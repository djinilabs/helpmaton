...
export const database = once(async (): Promise<DatabaseSchemaWithAtomicUpdate> => {
      ...
      try {
        ...
        for (const recordToPut of recordsToPut) {
          ...
          if (isCreate) {
            ...
            const { version: _, createdAt: __, ...recordWithoutVersion } = recordToPut as Record<string, unknown>;
            ...
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const { version: _, updatedAt: __, ...recordWithoutVersion } = recordToPut as Record<string, unknown>;
            validatedItem = parseItem(
              {
                ...existingRecord,
                ...recordWithoutVersion,
