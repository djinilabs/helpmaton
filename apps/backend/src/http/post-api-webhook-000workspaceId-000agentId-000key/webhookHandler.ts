...
import { convertTextToUIMessage } from "../utils/messageConversion";
import {
...
> {
  const { reasoningFromSteps, toolCallsFromResult, toolResultsFromResult, responseContent } =
    options;
...
async function logWebhookConversation(options: {
  ...
  awsRequestId?: string;
  context: ReturnType<typeof getContextFromRequestId>;
}): Promise<void> {
  const {
    db,
    workspaceId,
  ...
  console.log("[Webhook Handler] Messages being passed to startConversation:", {
...
export async function handleWebhookRequest(
  ...
  try {
    ...
    const assistantMessage: UIMessage = {
      role: "assistant",
      content: assistantContent.length > 0 ? assistantContent : responseContent || "",
