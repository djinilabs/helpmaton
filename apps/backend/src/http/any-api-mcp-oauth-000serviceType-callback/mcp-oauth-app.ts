  ...
  app.get(
    ...
    async (req, res, next) => {
      try {
        ...
        if (serviceType !== "google-drive" && serviceType !== "gmail" && serviceType !== "google-calendar" && serviceType !== "notion" && serviceType !== "github" && serviceType !== "linear" && serviceType !== "hubspot" && serviceType !== "slack") {
          const errorMsg = encodeURIComponent(
        ...
        try {
          if (serviceType === "google-drive") {
            ...
            tokenInfo = await exchangeGoogleCalendarCode(code);
          } else if (serviceType === "notion") {
            const { exchangeNotionCode } = await import(
              "../../utils/oauth/mcp/notion"
            );
            tokenInfo = await exchangeNotionCode(code);
          } else if (serviceType === "github") {
            const { exchangeGithubCode } = await import(
              "../../utils/oauth/mcp/github"
            );
            tokenInfo = await exchangeGithubCode(code);
          } else if (serviceType === "linear") {
            const { exchangeLinearCode } = await import(
