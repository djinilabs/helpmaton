...
describe("POST /api/workspaces/:workspaceId/agents/:agentId/spending-limits", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  async function callRouteHandler(
    ...
    const handler = async (
      ...
      try {
        ...
        const updated = (await mockAddSpendingLimit(
          db,
          workspaceId,
          { timeFrame, amount },
          agentId
  ...
  it("should successfully add daily spending limit to agent", async () => {
    ...
    mockAddSpendingLimit.mockResolvedValue(updatedAgent);
    ...
    expect(mockAddSpendingLimit).toHaveBeenCalledWith(
      mockDb,
      workspaceId,
  ...
  it("should successfully add weekly spending limit to agent", async () => {
    const mockDb = createMockDatabase();
    mockDatabase.mockResolvedValue(mockDb);
    ...
    const updatedAgent = {
      ...
      spendingLimits: [{ timeFrame: "weekly", amount: 500 }],
      createdAt: "2024-01-01T00:00:00Z",
      updatedAt: "2024-01-02T00:00:00Z",
    };
  ...
  it("should successfully add monthly spending limit to agent", async () => {
    const mockDb = createMockDatabase();
    mockDatabase.mockResolvedValue(mockDb);
    ...
    const updatedAgent = {
      ...
      systemPrompt: "You are a helpful assistant",
      notificationChannelId: null,
      spendingLimits: [{ timeFrame: "monthly", amount: 2000 }],
      createdAt: "2024-01-01T00:00:00Z",
      updatedAt: "2024-01-02T00:00:00Z",
    };
