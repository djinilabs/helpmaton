import { badRequest, forbidden, resourceGone } from "@hapi/boom";
import express from "express";
import { describe, it, expect, vi, beforeEach } from "vitest";

import {
  createMockRequest,
  createMockResponse,
  createMockDatabase,
} from "../../../utils/__tests__/test-helpers";

// Mock dependencies using vi.hoisted to ensure they're set up before imports
const { mockDatabase, mockSendNotification } = vi.hoisted(() => {
  return {
    mockDatabase: vi.fn(),
    mockSendNotification: vi.fn(),
  };
});

// Mock the modules
vi.mock("../../../../tables", () => ({
  database: mockDatabase,
}));

vi.mock("../../../../utils/notifications", () => ({
  sendNotification: mockSendNotification,
}));

describe("POST /api/workspaces/:workspaceId/channels/:channelId/test", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  async function callRouteHandler(
    req: Partial<express.Request>,
    res: Partial<express.Response>,
    next: express.NextFunction
  ) {
    // Extract the handler logic directly
    const handler = async (
      req: express.Request,
      res: express.Response,
      next: express.NextFunction
    ) => {
      try {
        const db = await mockDatabase();
        const workspaceResource = (req as { workspaceResource?: string })
          .workspaceResource;
        if (!workspaceResource) {
          throw badRequest("Workspace resource not found");
        }
        const workspaceId = req.params.workspaceId;
        const channelId = req.params.channelId;
        const channelPk = `output-channels/${workspaceId}/${channelId}`;

        const channel = await db["output_channel"].get(channelPk, "channel");
        if (!channel) {
          throw resourceGone("Channel not found");
        }

        if (channel.workspaceId !== workspaceId) {
          throw forbidden("Channel does not belong to this workspace");
        }

        // Send test message
        const testMessage = `✅ Test notification from Helpmaton\n\nThis is a test message to verify that your ${channel.name} channel is configured correctly. If you received this message, your channel setup is working!`;

        try {
          await mockSendNotification(channel, testMessage);
          res.json({
            success: true,
            message: "Test message sent successfully",
          });
        } catch (error) {
          const errorMessage =
            error instanceof Error ? error.message : String(error);
          throw badRequest(`Failed to send test message: ${errorMessage}`);
        }
      } catch (error) {
        next(error);
      }
    };

    await handler(req as express.Request, res as express.Response, next);
  }

  it("should send test message successfully", async () => {
    const mockDb = createMockDatabase();
    mockDatabase.mockResolvedValue(mockDb);

    const workspaceId = "workspace-123";
    const workspaceResource = `workspaces/${workspaceId}`;
    const channelId = "channel-456";
    const channelPk = `output-channels/${workspaceId}/${channelId}`;

    const mockChannel = {
      pk: channelPk,
      sk: "channel",
      workspaceId,
      name: "Discord Channel",
      type: "discord",
      config: {
        botToken: "bot-token-123",
        discordChannelId: "discord-channel-123",
      },
      createdAt: "2024-01-01T00:00:00Z",
    };

    const mockGet = vi.fn().mockResolvedValue(mockChannel);
    (mockDb as Record<string, unknown>)["output_channel"] = {
      get: mockGet,
    };

    mockSendNotification.mockResolvedValue(undefined);

    const req = createMockRequest({
      workspaceResource,
      params: {
        workspaceId,
        channelId,
      },
    });
    const res = createMockResponse();
    const next = vi.fn();

    await callRouteHandler(req, res, next);

    expect(mockGet).toHaveBeenCalledWith(channelPk, "channel");
    expect(mockSendNotification).toHaveBeenCalledWith(
      mockChannel,
      `✅ Test notification from Helpmaton\n\nThis is a test message to verify that your ${mockChannel.name} channel is configured correctly. If you received this message, your channel setup is working!`
    );
    expect(res.json).toHaveBeenCalledWith({
      success: true,
      message: "Test message sent successfully",
    });
    expect(next).not.toHaveBeenCalled();
  });

  it("should throw badRequest when workspace resource is missing", async () => {
    const mockDb = createMockDatabase();
    mockDatabase.mockResolvedValue(mockDb);

    const workspaceId = "workspace-123";
    const channelId = "channel-456";

    const req = createMockRequest({
      params: {
        workspaceId,
        channelId,
      },
    });
    const res = createMockResponse();
    const next = vi.fn();

    await callRouteHandler(req, res, next);

    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({
        output: expect.objectContaining({
          statusCode: 400,
          payload: expect.objectContaining({
            message: "Workspace resource not found",
          }),
        }),
      })
    );
    expect(mockSendNotification).not.toHaveBeenCalled();
    expect(res.json).not.toHaveBeenCalled();
  });

  it("should throw resourceGone when channel is not found", async () => {
    const mockDb = createMockDatabase();
    mockDatabase.mockResolvedValue(mockDb);

    const workspaceId = "workspace-123";
    const workspaceResource = `workspaces/${workspaceId}`;
    const channelId = "channel-456";
    const channelPk = `output-channels/${workspaceId}/${channelId}`;

    const mockGet = vi.fn().mockResolvedValue(null);
    (mockDb as Record<string, unknown>)["output_channel"] = {
      get: mockGet,
    };

    const req = createMockRequest({
      workspaceResource,
      params: {
        workspaceId,
        channelId,
      },
    });
    const res = createMockResponse();
    const next = vi.fn();

    await callRouteHandler(req, res, next);

    expect(mockGet).toHaveBeenCalledWith(channelPk, "channel");
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({
        output: expect.objectContaining({
          statusCode: 410,
          payload: expect.objectContaining({
            message: "Channel not found",
          }),
        }),
      })
    );
    expect(mockSendNotification).not.toHaveBeenCalled();
    expect(res.json).not.toHaveBeenCalled();
  });

  it("should throw forbidden when channel belongs to different workspace", async () => {
    const mockDb = createMockDatabase();
    mockDatabase.mockResolvedValue(mockDb);

    const workspaceId = "workspace-123";
    const workspaceResource = `workspaces/${workspaceId}`;
    const channelId = "channel-456";
    const channelPk = `output-channels/${workspaceId}/${channelId}`;

    const mockChannel = {
      pk: channelPk,
      sk: "channel",
      workspaceId: "different-workspace-789", // Different workspace
      name: "Discord Channel",
      type: "discord",
      config: {
        botToken: "bot-token-123",
        discordChannelId: "discord-channel-123",
      },
      createdAt: "2024-01-01T00:00:00Z",
    };

    const mockGet = vi.fn().mockResolvedValue(mockChannel);
    (mockDb as Record<string, unknown>)["output_channel"] = {
      get: mockGet,
    };

    const req = createMockRequest({
      workspaceResource,
      params: {
        workspaceId,
        channelId,
      },
    });
    const res = createMockResponse();
    const next = vi.fn();

    await callRouteHandler(req, res, next);

    expect(mockGet).toHaveBeenCalledWith(channelPk, "channel");
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({
        output: expect.objectContaining({
          statusCode: 403,
          payload: expect.objectContaining({
            message: "Channel does not belong to this workspace",
          }),
        }),
      })
    );
    expect(mockSendNotification).not.toHaveBeenCalled();
    expect(res.json).not.toHaveBeenCalled();
  });

  it("should throw badRequest when sendNotification fails", async () => {
    const mockDb = createMockDatabase();
    mockDatabase.mockResolvedValue(mockDb);

    const workspaceId = "workspace-123";
    const workspaceResource = `workspaces/${workspaceId}`;
    const channelId = "channel-456";
    const channelPk = `output-channels/${workspaceId}/${channelId}`;

    const mockChannel = {
      pk: channelPk,
      sk: "channel",
      workspaceId,
      name: "Discord Channel",
      type: "discord",
      config: {
        botToken: "bot-token-123",
        discordChannelId: "discord-channel-123",
      },
      createdAt: "2024-01-01T00:00:00Z",
    };

    const mockGet = vi.fn().mockResolvedValue(mockChannel);
    (mockDb as Record<string, unknown>)["output_channel"] = {
      get: mockGet,
    };

    const errorMessage = "Discord bot token is missing";
    mockSendNotification.mockRejectedValue(new Error(errorMessage));

    const req = createMockRequest({
      workspaceResource,
      params: {
        workspaceId,
        channelId,
      },
    });
    const res = createMockResponse();
    const next = vi.fn();

    await callRouteHandler(req, res, next);

    expect(mockGet).toHaveBeenCalledWith(channelPk, "channel");
    expect(mockSendNotification).toHaveBeenCalled();
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({
        output: expect.objectContaining({
          statusCode: 400,
          payload: expect.objectContaining({
            message: `Failed to send test message: ${errorMessage}`,
          }),
        }),
      })
    );
    expect(res.json).not.toHaveBeenCalled();
  });

  it("should throw badRequest when sendNotification fails with string error", async () => {
    const mockDb = createMockDatabase();
    mockDatabase.mockResolvedValue(mockDb);

    const workspaceId = "workspace-123";
    const workspaceResource = `workspaces/${workspaceId}`;
    const channelId = "channel-456";
    const channelPk = `output-channels/${workspaceId}/${channelId}`;

    const mockChannel = {
      pk: channelPk,
      sk: "channel",
      workspaceId,
      name: "Discord Channel",
      type: "discord",
      config: {
        botToken: "bot-token-123",
        discordChannelId: "discord-channel-123",
      },
      createdAt: "2024-01-01T00:00:00Z",
    };

    const mockGet = vi.fn().mockResolvedValue(mockChannel);
    (mockDb as Record<string, unknown>)["output_channel"] = {
      get: mockGet,
    };

    const errorMessage = "Network timeout";
    mockSendNotification.mockRejectedValue(errorMessage);

    const req = createMockRequest({
      workspaceResource,
      params: {
        workspaceId,
        channelId,
      },
    });
    const res = createMockResponse();
    const next = vi.fn();

    await callRouteHandler(req, res, next);

    expect(mockGet).toHaveBeenCalledWith(channelPk, "channel");
    expect(mockSendNotification).toHaveBeenCalled();
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({
        output: expect.objectContaining({
          statusCode: 400,
          payload: expect.objectContaining({
            message: `Failed to send test message: ${errorMessage}`,
          }),
        }),
      })
    );
    expect(res.json).not.toHaveBeenCalled();
  });

  it("should include channel name in test message", async () => {
    const mockDb = createMockDatabase();
    mockDatabase.mockResolvedValue(mockDb);

    const workspaceId = "workspace-123";
    const workspaceResource = `workspaces/${workspaceId}`;
    const channelId = "channel-456";
    const channelPk = `output-channels/${workspaceId}/${channelId}`;

    const channelName = "My Custom Discord Channel";
    const mockChannel = {
      pk: channelPk,
      sk: "channel",
      workspaceId,
      name: channelName,
      type: "discord",
      config: {
        botToken: "bot-token-123",
        discordChannelId: "discord-channel-123",
      },
      createdAt: "2024-01-01T00:00:00Z",
    };

    const mockGet = vi.fn().mockResolvedValue(mockChannel);
    (mockDb as Record<string, unknown>)["output_channel"] = {
      get: mockGet,
    };

    mockSendNotification.mockResolvedValue(undefined);

    const req = createMockRequest({
      workspaceResource,
      params: {
        workspaceId,
        channelId,
      },
    });
    const res = createMockResponse();
    const next = vi.fn();

    await callRouteHandler(req, res, next);

    expect(mockSendNotification).toHaveBeenCalledWith(
      mockChannel,
      `✅ Test notification from Helpmaton\n\nThis is a test message to verify that your ${channelName} channel is configured correctly. If you received this message, your channel setup is working!`
    );
    expect(res.json).toHaveBeenCalledWith({
      success: true,
      message: "Test message sent successfully",
    });
  });
});
