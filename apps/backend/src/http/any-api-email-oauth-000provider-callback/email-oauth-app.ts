  ...
  app.get(
    ...
    async (req, res, next) => {
      try {
        ...
        const redirectBaseUrl = process.env.FRONTEND_URL || "http://localhost:5173";
        ...
        if (error) {
          ...
          let workspaceId = "";
          if (state) {
            const { validateAndExtractStateToken } = await import("../../utils/oauth/common");
        ...
        if (!["gmail", "outlook"].includes(provider)) {
          const errorMsg = encodeURIComponent('provider must be "gmail" or "outlook"');
          return res.redirect(
            `${redirectBaseUrl}/workspaces/${workspaceId}/email-oauth-callback?success=false&error=${errorMsg}&provider=${provider}`
          );
        }
        ...
        try {
          ...
          return res.redirect(`${redirectBaseUrl}/api/auth/signin?callbackUrl=${returnUrl}`);
        }
        ...
        try {
          ...
          if (!authorized) {
            const errorMsg = encodeURIComponent("You don't have permission to modify this workspace");
            return res.redirect(
              `${redirectBaseUrl}/workspaces/${workspaceId}/email-oauth-callback?success=false&error=${errorMsg}&provider=${provider}`
            );
          }
        ...
        let tokenInfo: { accessToken: string; refreshToken: string; expiresAt: string; email?: string };
        try {
          if (provider === "gmail") {
            const { exchangeGmailCode } = await import("../../utils/oauth/gmail");
            tokenInfo = await exchangeGmailCode(code);
          } else {
            const { exchangeOutlookCode } = await import("../../utils/oauth/outlook");
          ...
          console.error(`[OAuth Callback] Token exchange failed for ${provider}:`, error);
          const errorMsg = encodeURIComponent(
            error instanceof Error ? error.message : "Failed to exchange OAuth code for tokens"
          );
          return res.redirect(
            `${redirectBaseUrl}/workspaces/${workspaceId}/email-oauth-callback?success=false&error=${errorMsg}&provider=${provider}`
          );
        ...
        if (!tokenInfo.accessToken || !tokenInfo.refreshToken || !tokenInfo.expiresAt) {
          console.error(`[OAuth Callback] Missing required token fields for ${provider}:`, {
            hasAccessToken: !!tokenInfo.accessToken,
            hasRefreshToken: !!tokenInfo.refreshToken,
            hasExpiresAt: !!tokenInfo.expiresAt,
            hasEmail: !!tokenInfo.email,
        ...
        if (tokenInfo.email !== undefined && tokenInfo.email !== null && tokenInfo.email !== "") {
