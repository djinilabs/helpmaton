...
import { createApp } from "./email-oauth-app";
...
const createHandler = async (): Promise<APIGatewayProxyHandlerV2> => {
  ...
  try {
    ...
    console.error("[email-oauth] Error creating app:", error);
    if (error instanceof Error) {
      console.error("[email-oauth] Error stack:", error.stack);
    }
    throw error;
  }
};
  ...
  handlingErrors(
    async (...args: Parameters<APIGatewayProxyHandlerV2>) => {
      try {
        ...
        console.error("[email-oauth] Error in handler:", error);
        if (error instanceof Error) {
          console.error("[email-oauth] Error stack:", error.stack);
        }
        throw error;
