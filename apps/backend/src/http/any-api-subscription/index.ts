...
import { createApp } from "../any-api-subscription-catchall/subscription-app";

let cachedHandler: APIGatewayProxyHandlerV2 | undefined;

const createHandler = async (): Promise<APIGatewayProxyHandlerV2> => {
  if (cachedHandler) {
    console.log("[subscription] Using cached handler");
    return cachedHandler;
  }
  try {
    console.log("[subscription] Creating app...");
    const app = createApp(); // createApp is not async, so no await needed
    console.log("[subscription] App created, setting up serverless-express");
    ...
    console.log("[subscription] Handler created and cached");
    return handler;
  } catch (error) {
    console.error("[subscription] Error creating app:", error);
...
export const handler: APIGatewayProxyHandlerV2 = handlingErrors(
  async (...args: Parameters<APIGatewayProxyHandlerV2>) => {
    ...
    try {
      ...
      console.log("[subscription] Handler created, calling with args");
      const result = await h(...args);
      console.log(
        "[subscription] Handler returned result:",
      ...
      console.error("[subscription] Error in handler:", error);
      if (error instanceof Error) {
        console.error("[subscription] Error stack:", error.stack);
      }
      throw error;
