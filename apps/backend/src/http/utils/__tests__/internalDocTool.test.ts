...
describe("internalDocTool", () => {
  it("returns content for a valid doc id", () => {
    const state: ReadInternalDocState = { callCount: 0 };
    const firstId = INTERNAL_DOCS_INDEX[0]?.id;
    expect(firstId).toBeDefined();
    const result = executeReadInternalDoc(state, firstId!);
    expect(typeof result).toBe("string");
    expect(result.startsWith("#")).toBe(true);
    expect(result).not.toContain('"error"');
  });

  it("returns Document not found error for unknown doc id", () => {
    const state: ReadInternalDocState = { callCount: 0 };
    const result = executeReadInternalDoc(state, "nonexistent-doc-id-12345");
    const parsed = JSON.parse(result) as { error: string; docId: string; hint: string };
  ...
  it("returns Document not found for empty doc id", () => {
    const state: ReadInternalDocState = { callCount: 0 };
    const result = executeReadInternalDoc(state, "   ");
    const parsed = JSON.parse(result) as { error: string };
    expect(parsed.error).toBe("Document not found");
  });

  it("after 3 calls, 4th call returns max attempts error", () => {
    const state: ReadInternalDocState = { callCount: 0 };
    const validId = Object.keys(INTERNAL_DOCS_CONTENT)[0];
    expect(validId).toBeDefined();

    executeReadInternalDoc(state, validId!);
    executeReadInternalDoc(state, validId!);
    executeReadInternalDoc(state, validId!);
    const fourth = executeReadInternalDoc(state, validId!);

    const parsed = JSON.parse(fourth) as { error: string };
    expect(parsed.error).toBe(MAX_ATTEMPTS_ERROR);
  });

  it("normalizes docId: accepts Getting-Started and getting-started.md", () => {
    const state: ReadInternalDocState = { callCount: 0 };
    const canonicalId = Object.keys(INTERNAL_DOCS_CONTENT)[0];
    if (!canonicalId) return;
    const result1 = executeReadInternalDoc(state, canonicalId);
    expect(result1.startsWith("#")).toBe(true);
    const state2: ReadInternalDocState = { callCount: 0 };
    const withSuffix = `${canonicalId}.md`;
    const result2 = executeReadInternalDoc(state2, withSuffix);
    expect(result2).toBe(result1);
    const state3: ReadInternalDocState = { callCount: 0 };
    const mixedCase = canonicalId.split("-").map((s, i) => (i === 0 ? s : s.charAt(0).toUpperCase() + s.slice(1))).join("-");
    const result3 = executeReadInternalDoc(state3, mixedCase);
    expect(result3).toBe(result1);
  });

  it("state is per-request: two states do not share call count", () => {
    const state1: ReadInternalDocState = { callCount: 0 };
    const state2: ReadInternalDocState = { callCount: 0 };
    const validId = Object.keys(INTERNAL_DOCS_CONTENT)[0]!;

    executeReadInternalDoc(state1, validId);
    executeReadInternalDoc(state1, validId);
    executeReadInternalDoc(state1, validId);

    const result2 = executeReadInternalDoc(state2, validId);
