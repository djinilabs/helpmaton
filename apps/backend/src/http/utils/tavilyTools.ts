...
export function createScrapeFetchTool(
  ...
  const description =
    "Scrape web pages using Puppeteer with residential proxies to extract Accessibility Object Model (AOM) as XML (browser-based fetch). This tool allows you to get the full content structure from any web page, including JavaScript-rendered content. The service uses stealth techniques to avoid detection and attempts to solve captchas automatically. IMPORTANT: This service can take significantly longer to fetch content compared to other providers (may take 30-60 seconds or more) due to the stealth measures and captcha solving. Use this when you need to read and understand the complete structure and content of a specific URL, especially for JavaScript-rendered pages. CRITICAL REQUIREMENTS: (1) You MUST provide the 'url' parameter - it is REQUIRED and cannot be empty. (2) The 'url' parameter must be a valid URL starting with http:// or https://. (3) When the user asks you to scrape or fetch content from a URL, IMMEDIATELY call this tool with the required 'url' parameter. Example: If user says 'scrape https://example.com/article', call fetch_url with {url: 'https://example.com/article'}. The tool uses residential proxies to access web pages and returns the AOM (Accessibility Object Model) structure as XML. Cost: $0.005 per call.";
