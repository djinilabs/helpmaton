...
export const createIntegrationDiscordCommandSchema = z.object({
  commandName: z.string().min(1, "commandName is required and must be a string"),
}).strict();

export const createIntegrationSlackManifestSchema = z.object({
  agentId: z.string().min(1, "agentId is required and must be a string"),
...
export const createSpendingLimitSchema = z.object({
  ...
  amount: z
    .number()
    .int()
    .nonnegative("amount is required and must be a non-negative integer (millionths)"),
}).strict();
...
export const createAgentSpendingLimitSchema = z.object({
  timeFrame: z.enum(["daily", "weekly", "monthly"]),
  amount: z
    .number()
    .int()
...
export const updateAgentSpendingLimitSchema = z.object({
  amount: z
    .number()
    .int()
    .nonnegative("amount is required and must be a non-negative integer (millionths)"),
...
export const createStreamServerSchema = z.object({
  allowedOrigins: z
    .array(originSchema)
    .min(1, "allowedOrigins must be an array with at least one origin"),
}).strict();

export const updateStreamServerSchema = z.object({
  allowedOrigins: z
    .array(originSchema)
    .min(1, "allowedOrigins must be an array with at least one origin"),
}).strict();
...
export const purchaseCreditsSchema = z.object({
  amount: z
    ...
    .refine((val) => val >= 1, { message: "Minimum purchase amount is 1 USD" })
    .refine(
...
export const trialCreditRequestSchema = z.object({
