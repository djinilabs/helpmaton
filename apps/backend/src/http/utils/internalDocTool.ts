 ...
 * Content is lazy-loaded from internalDocsContent when first needed.
 */
 ...
 * Loads INTERNAL_DOCS_CONTENT lazily on first use.
 */
export async function executeReadInternalDoc(
  ...
  try {
    ...
    const { INTERNAL_DOCS_CONTENT } = await import(
      "../../utils/internalDocsContent"
    );
    ...
    if (content === undefined) {
      return JSON.stringify({
        ...
        docId: normalizedId,
        hint: "Use only doc IDs from the internal docs index.",
        validIds: Object.keys(INTERNAL_DOCS_CONTENT),
      });
    }
