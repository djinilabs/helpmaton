...
export const UsageDashboard: FC<UsageDashboardProps> = ({
  ...
  return (
    <div className={`${showBorder ? "bg-white rounded-2xl shadow-medium p-8 border border-neutral-200" : ""} mb-8`}>
      <div className={`flex flex-col sm:flex-row items-start sm:items-center mb-6 gap-4 ${showBorder ? 'justify-between' : 'justify-end'}`}>
        {showBorder && <h2 className="text-3xl font-bold text-neutral-900 tracking-tight">{title}</h2>}
        <div className="flex flex-wrap gap-3 items-center">
          {onDateRangeChange && (
            <select
              value={dateRangePreset}
              onChange={(e) => onDateRangeChange(e.target.value as DateRangePreset)}
              className="border border-neutral-300 rounded-xl px-4 py-2.5 text-sm font-semibold bg-white text-neutral-900 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200"
            >
