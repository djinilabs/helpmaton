...
- **S3 ECONNREFUSED 127.0.0.1:4568 in production (Sentry 98403916, 2026-02-23)**: OpenRouter cost verification queue (and any code using getS3Client/getS3ObjectBody) was using the local S3 endpoint in production because `isLocal` was true when ARC_ENV or NODE_ENV were unset or non-production in Lambda. **Fix**: In `s3.ts`, added `isLocalS3Environment()` that returns false when `AWS_LAMBDA_FUNCTION_NAME` is set (Lambda runtime always sets this). So in Lambda we always use real AWS S3; localhost is only used when not running in Lambda and ARC_ENV=testing or (ARC_ENV !== "production" && NODE_ENV !== "production"). Used in getS3Client, isLocalS3 (resolveConversationFileUrl), and getAwsSdkS3Client. Exported for testing; added unit tests in s3.test.ts. Typecheck, lint, and s3 + conversationRecords tests pass.
