...
- **Tool response size limits and pagination – review and improve (2026-02-17)**: (1) **Shared schemas**: Extracted `listLimitSchema` and `listCursorSchema` to `toolSchemas.ts`; workspaceAgentTools and agentConfigTools import from there. (2) **list_documents**: When `folder` is set, use DynamoDB `FilterExpression: "folderPath = :folderPath"` so pagination returns a proper page of matching documents instead of filtering in memory. (3) **DatabaseSchema**: Added `agent-eval-judge` and `agent-eval-result` to `DatabaseSchema` and `TableRecord` in schema.ts so `db["agent-eval-judge"]` is typed; removed cast in agentConfigTools. (4) **applyQueryLimit**: Strip trailing semicolon before matching/appending SOQL LIMIT; exported for testing; added unit tests (no LIMIT, cap existing LIMIT, trailing semicolon). Typecheck and tests pass. Lint reports 3 pre-existing errors in linearTools.ts and zendesk/client.ts (unrelated).
