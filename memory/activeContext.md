...
- **ECR image retention reduced (2026-03-02)**: To cut AWS ECR spend, image retention was reduced. (1) **Build script** (`scripts/build-and-push-lambda-images.sh`): After ensuring the ECR repo exists, the script now applies a lifecycle policy that keeps only the last **3** images per image type (tag prefix `lancedb-`, `puppeteer-`, etc.); policy is applied on every build. (2) **container-images plugin**: `ensureEcrRepository` lifecycle policy updated from "keep 10 with prefix latest" to keep 3 each for `lancedb-` and `puppeteer-`. (3) **Cleanup workflow** (`.github/workflows/cleanup-ecr-images.yml`) and **cleanup script** (`scripts/cleanup-ecr-images.mjs`): Default production retention reduced from 5/15 to **3**. Docs: `docs/deployment.md`, `docs/ecr-image-cleanup.md` updated. To apply the new lifecycle policy to the existing repo immediately, run a deploy (build-and-push) or manually put the lifecycle policy via AWS CLI.

- **Memory extraction ZodError (Sentry 99774263, PR 298 staging, 2026-02-28)**: `memory_operations[1].object` expected string, received number. LLMs sometimes return numbers for object (e.g. quantities like "3 children"). Fix: accept string or number for subject, predicate, and object via `stringOrNumberSchema` (union + transform to string + non-empty refine) in memoryExtraction.ts. New unit test: "accepts numeric object (and subject/predicate) from LLM and coerces to string". Typecheck, lint, and memoryExtraction tests pass.

- **Memory extraction retry on parse failure (2026-02-28)**: When extraction parse/validation fails, we now feed the error back to the LLM and retry up to `MAX_MEMORY_EXTRACTION_ATTEMPTS` (2). In memoryExtraction.ts: loop over attempts; on empty response or parseExtractionResponse throw we append assistant + user message (buildRetryUserMessage) and continue; each attempt has its own reservation and credit adjustment. Unit tests: "refeeds parse error and retries; succeeds on second attempt", "throws after max attempts when parse keeps failing". **Review-and-improve**: (1) Truncate validation error in refeed to `RETRY_ERROR_MESSAGE_MAX_LENGTH` (500 chars) to avoid huge context. (2) Comment above retry loop; log when starting retry (attempt > 1). (3) When exhausting retries throw `new Error("...after N attempts", { cause: parseError })` so Sentry gets clear message and cause chain; test asserts message and cause. (4) writeMemory: set `extra.retriesExhausted` and tag `retries_exhausted: "true"` when caught error message matches "failed after N attempts" so Sentry can filter. **Second review-and-improve**: (1) Wording: "refeed" → "feed back" in comments. (2) DRY: `throwExhaustedEmptyResponse()` helper for both empty/no-result exhaustion paths. (3) `RESPONSE_PREVIEW_MAX_LENGTH` constant; JSDoc on `extractConversationMemory` (charge/refund policy).
...
- **E2E tests on main – fixes (2026-02-24)**: (1) **Backend ready race**: Global-setup was resolving as soon as "Sandbox Started" appeared, before the plugin-typescript finished building handlers. That caused `checkServiceReady` to hit the server and trigger Lambda init error for `any-catchall` (handler not built yet). Fix: only consider backend ready when `compilationComplete` is true (look for "[plugin-typescript] Compiled ", "Sandbox Ran Sandbox startup plugins", or "File watcher now looking"). (2) **localStorage SecurityError**: `LoginPage.goto` and login.spec beforeEach cleared localStorage/sessionStorage via `page.evaluate()`; when the document was in an opaque origin or error page, that threw SecurityError and failed the test. Fix: wrap clear in try-catch inside the evaluate callback in `tests/e2e/pages/login-page.ts` and `tests/e2e/login.spec.ts`. Run full `pnpm test:e2e` to confirm all specs pass (avoid running a subset of specs without full global setup).

- **Free plan / Lemon Squeezy – review and improve (2026-02-24)**: After implementing no-error-for-free-plan (portal/cancel no-op, hide billing buttons): (1) Backend: added comments for free-plan no-op branches in subscription-app.ts (cancel and portal). (2) Test: fixed callPortalHandler indentation in subscription-modify-cancel.test.ts. (3) Frontend: portal refetch handlers now have .catch() with toast.error so network failures don’t surface as unhandled rejections; cancel button gated with lemonSqueezySubscriptionId so it only shows when there’s a Lemon Squeezy subscription (consistency with Manage payment). Typecheck and subscription tests pass.

- **Discord API "Invalid Form Body" on send_notification (Sentry 98397906, 2026-02-23)**: Production agent-schedule-queue failed when sending a Discord notification. **Root cause**: Discord allows message content only 1–2000 characters; we sent content as-is, so long notifications (e.g. from scheduled/onboarding-created agents) triggered 400 Invalid Form Body. **Fix**: (1) `discord.ts`: Added `DISCORD_MESSAGE_MAX_LENGTH`, `normalizeDiscordContent()` (trim, reject empty, truncate to 2000 with "…"); `sendDiscordMessage` uses it before POST. (2) 400 responses: include `errorData.errors` in thrown message for debugging. (3) `send_notification` tool: Zod schema `content.max(2000)` and description mentions 2000-char limit; import `DISCORD_MESSAGE_MAX_LENGTH` from discord. (4) Unit tests: `discord.test.ts` for normalizeDiscordContent (trim, empty, truncate) and sendDiscordMessage (normalization, empty throw, 400 with errors, 403). Typecheck and discord + notifications tests pass.

- **S3 env: local sandbox, staging, production (2026-02-25)**: Verified and documented. Real Lambda detected via `AWS_LAMBDA_LOG_GROUP_NAME` (/aws/lambda/...); sandbox has no log group and ARC_ENV=testing so uses local s3rver. Added test block "S3 environment: local sandbox, staging, production" with explicit cases for sandbox (local S3), staging (real S3), production (real S3), plus fallbacks when log group unset. JSDoc in s3.ts lists the three environments.
- **S3 ECONNREFUSED 127.0.0.1:4568 in production (Sentry 98403916, 2026-02-23)**: OpenRouter cost verification queue was using local S3 in production. **Fix**: Real Lambda detected via `AWS_LAMBDA_LOG_GROUP_NAME`; `isLocalS3Environment()` + safeguard in getS3Client/getAwsSdkS3Client force AWS S3 when endpoint is localhost and in real Lambda. Sandbox (no log group, ARC_ENV=testing) keeps local s3rver.

- **S3 JSON parse "Unexpected end of JSON input" in enrich-from-s3 (Sentry 98327573, 2026-02-22)**: Production conversation-records enrichment failed when S3 message blobs were empty or contained invalid/truncated JSON. **Fix**: In `fetchMessagesFromS3Key` (conversationRecords.ts): (1) return `[]` when S3 body length is 0; (2) catch `SyntaxError` from `JSON.parse`, log warning, and return `[]` so enrichment never throws. Added unit tests: "returns record with empty messages when S3 object body is empty" and "returns record with empty messages when S3 object contains invalid JSON". Typecheck, lint, and conversationRecords tests pass.

- **Schedule queue: always persist user + assistant when there is a response (2026-02-22)**: Scheduled runs could sometimes be stored with only the user message (schedule prompt) when `agentResult.observerEvents` was missing/empty. Fix: use `buildConversationMessagesFromObserver` whenever we have observer events **or** assistant text (`agentResult.text`), with `observerEvents: agentResult.observerEvents ?? []`, so we always save at least [userMessage, assistantMessage] for successful runs. Fallback to `[userMessage]` only when there are no observer events and no assistant text. **Review-and-improve**: (1) Comment in schedule queue explaining why we use observer/fallbacks (avoid empty conversations). (2) New llmObserver test "builds user + assistant from fallbacks when observerEvents is empty (e.g. schedule queue)" locking in that empty events + fallbackInputMessages + fallbackAssistantText yields [user, assistant]. Typecheck and tests pass.

- **OpenRouter embedding "Invalid embedding response format" (Sentry 98214067, 2026-02-22)**: Production `agent-temporal-grain-queue` failed when OpenRouter returned HTTP 200 with an unexpected body (missing or invalid `data` array). **Root cause**: OpenRouter or upstream (thenlper/gte-base) can return 200 with (1) an error-shaped body `{ error: { message } }` or (2) a different schema. We only validated `data` and threw a generic "Invalid embedding response format" with no response body in Sentry. **Fix**: (1) Treat 200 with `body.error` as an error: throw with formatted message and statusCode 502 so it’s retryable. (2) Include response preview (first 500 chars) in the "Invalid embedding response format" error message so Sentry captures the actual response. (3) Tightened timeout detection: only treat as timeout/abort when `name === "AbortError"` or message is "Operation aborted" or starts with "Embedding generation timed out" (avoids misclassifying upstream messages containing "timeout"). New test: 200 with error body throws with clear message and 502. Typecheck and embedding tests pass.
