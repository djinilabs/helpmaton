...
- **Service worker RangeError fix – review and improve (2026-02-24)**: (1) **Generalized re-entrancy guard**: Replaced root-only `handlingRootFetch` with an `inFlightUrls` Set so both `cacheRootDocument()` and `cacheFirst()` are protected—on browsers that re-dispatch our own `fetch()` (e.g. Chrome Mobile iOS), static-asset requests could also recurse and blow the stack. (2) **Single check**: Fetch handler now skips responding when `request.url` is in `inFlightUrls`; root and static paths add/remove URL in try/finally. (3) **File comment**: Added top-of-file JSDoc explaining the guard and the RangeError cause. Typecheck passes.
...
- **S3 ECONNREFUSED 127.0.0.1:4568 in production (Sentry 98403916, 2026-02-23)**: OpenRouter cost verification queue was using local S3 in production. **Fix**: `isLocalS3Environment()` returns false only when `AWS_LAMBDA_FUNCTION_NAME` is set **and** `ARC_ENV !== "testing"` (so sandbox/E2E keep using local s3rver; production/staging Lambdas use real AWS S3). CI E2E was failing because sandbox also sets AWS_LAMBDA_FUNCTION_NAME; without the ARC_ENV check, E2E used production S3 and document upload failed. Added test "returns true when in Lambda but ARC_ENV is testing (sandbox/E2E)". Typecheck and s3 tests pass.
