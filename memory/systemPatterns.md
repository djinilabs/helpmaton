...
- **Memory extraction**: Per-agent extraction can summarize full conversations and emit graph fact operations; extraction validates credits before LLM calls, writes conversation-level working memory with deterministic IDs via SQS updates, and persists graph facts through DuckPGQ. On parse/validation failure (e.g. ZodError), the error is refed to the LLM (validation error message truncated to 500 chars) and extraction is retried up to `MAX_MEMORY_EXTRACTION_ATTEMPTS` (2); each attempt has its own reservation and credit adjustment. Exhausted retries throw with cause chain; writeMemory tags Sentry with `retries_exhausted` when falling back after max attempts.
