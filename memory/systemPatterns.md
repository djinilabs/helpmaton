...
- **Embeddings**: Use OpenRouter embeddings (`thenlper/gte-base`) via `@openrouter/sdk`; embeddings can use workspace OpenRouter keys when BYOK is configured, otherwise fall back to the system `OPENROUTER_API_KEY`.
