...
- **List tool pagination**: DynamoDB list tools (workspace agents, agent config) use shared `listLimitSchema` and `listCursorSchema` from `toolSchemas.ts` (limit default 50, max 200; cursor optional). Use `queryPaginated` and return `nextCursor`, `hasMore`. When filtering list results (e.g. by folder), add a DynamoDB `FilterExpression` to the query so each page contains only matching items; avoid filtering in memory after fetch so pagination stays correct.
