...
- **DynamoDB**: Single-table design with GSIs. Items written via table API are deep-cleaned (nested undefined removed) so @aws-lite marshalling never sees undefined; otherwise PutItem/TransactWriteItems can throw "Pass options.removeUndefinedValues=true...". See `deepClean` in tableApi.ts.
...
- **Embeddings**: Use OpenRouter embeddings (`thenlper/gte-base`) via direct `fetch` to `https://openrouter.ai/api/v1/embeddings` (not `@openrouter/sdk`) so we control response validation; we only require `data[0].embedding` to be an array of numbers, avoiding SDK ResponseValidationError when OpenRouter returns 200 with a non-standard or error-shaped body. Embeddings can use workspace OpenRouter keys when BYOK is configured, otherwise fall back to the system `OPENROUTER_API_KEY`. Background embedding generation (SQS queue) reserves/adjusts credits when using the system key and skips charges for BYOK.
...
- **Memory extraction**: Per-agent extraction can summarize full conversations and emit graph fact operations; extraction validates credits before LLM calls, writes conversation-level working memory with deterministic IDs via SQS updates, and persists graph facts through DuckPGQ. On parse/validation failure (e.g. ZodError), the error is fed back to the LLM (validation error message truncated to 500 chars) and extraction is retried up to `MAX_MEMORY_EXTRACTION_ATTEMPTS` (2); each attempt has its own reservation and credit adjustment. We charge for every LLM response (including empty or parse failure); we only refund when the provider completely fails (no response / non-OK status, i.e. generateText throws). Exhausted retries throw with cause chain; writeMemory tags Sentry with `retries_exhausted` when falling back after max attempts.
...
- **PostHog user identification (backend)**: Centralized in auth middleware. `ensurePostHogIdentityFromRequest(req)` is called from `requireAuth` / `requireAuthOrSession` (workspaces and subscription apps) after setting `req.userRef` and `req.session`. We do not identify in the signIn callback; identification is done by the frontend (PostHogProvider) and by auth middleware for API requests. In authenticated routes use `trackEvent(name, properties)` or `trackBusinessEvent(feature, action, properties)` without passing `req`; user is taken from request context. Pass `req` only when the handler did not go through that middleware, or pass `properties.user_id` in non-request flows (e.g. webhooks, queues).
...
- **PostHog unauthenticated**: We never send the `email` key or identification events when the user is not authenticated. For unauthenticated contexts, backend `trackEvent` sends any unverified email as `allegedEmail` (not `email`) so PostHog does not treat it as identification.
...
- **Modals (accessibility)**: Use `useModalFocus(panelRef, isOpen)` for focus trap and restore; set `role="dialog"`, `aria-modal="true"`, `aria-labelledby` (pointing to the title `id`), and `tabIndex={-1}` on the panel. Restore only runs if the previous element is still in the document. Apply `.modal-panel` for safe-area padding and full-bleed on small viewports; primary/close actions use `.touch-target`.
